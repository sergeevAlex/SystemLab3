...
public class Algorithm {
    public static ArrayList<List<Comp>> optim(List<Comp> mass) {
        int comp = 1024;
        ...
        while (!mass.isEmpty()) {
            ...
            if (fortime.isEmpty()) { //Этот процесс можно выполнить только последовательно!
                mass.remove(0);
                counter++;
            } else {
                ...
                while(!fortime.isEmpty()){
                    det = det - vrem.getMemory();
                    fortime.clear();
                    for (int i = 1; i < mass.size(); i++) {
                        if (mass.get(i).getMemory() <= det) {
                            fortime.add(mass.get(i));
